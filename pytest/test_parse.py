...
def test_parse_paragraph():
    ...
    parser = MarkdownParser(md)
    assert len(parser.blocks) == 1
    assert parser.to_html() == "<p>This is a paragraph.</p>"
...
def test_parse_code_block():
    ...
    parser = MarkdownParser(md)
    assert len(parser.blocks) == 1
    assert parser.to_html() == "<pre><code>\nimport somepackage\n</code></pre>"
...
"""
    ...
    expected = """<h1>Welcome to My Blog</h1>
...
"""
    parser = MarkdownParser(md)
    html = parser.to_html()
    expected = """<h1>Project Overview</h1>
